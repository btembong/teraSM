...
const PROGRAMS = [
  ...
  'Accounting', 'Economics', 'Medicine', 'Nursing', 'Law', 'Education', 'Arts & Humanities',
  'Social Sciences', 'Mathematics', 'Physics', 'Chemistry', 'Biology', 'Other',
]
...
type Step = 1 | 2 | 3 | 4 | 5
type DocFile = { docType: string; fileName: string; fileSize: number; file: File }
...
export default function ApplyPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const handleFile = (docType: string, file: File | null) => {
    ...
    setDocs(d => ({ ...d, [docType]: { docType, fileName: file.name, fileSize: file.size, file } }))
  }
  ...
  const submit = async () => {
    ...
    setSubmitting(true)
    try {
      const docList = Object.values(docs).map(d => ({
        docType:  d!.docType,
        fileName: d!.fileName,
        fileSize: d!.fileSize,
        fileUrl:  '', // R2 upload would go here
      }))
            ...
            <div className="space-y-4">
              ...
              {step === 3 && (
                <>
                  ...
                  <p className="text-xs text-gray-500">Upload clear, legible copies of your documents. Accepted formats: JPG, PNG, PDF.</p>
                  <div className="space-y-3">
                    {DOC_SLOTS.map(slot => {
                      const uploaded = docs[slot.key]
                      return (
                        <div key={slot.key} className={`border rounded-xl p-3 ${uploaded ? 'border-green-200 bg-green-50' : 'border-gray-200'}`}>
                          <div className="flex items-center justify-between mb-2">
                            ...
                            {uploaded && (
                              <button onClick={() => { handleFile(slot.key, null); if (fileRefs.current[slot.key]) fileRefs.current[slot.key]!.value = '' }} className="text-gray-400 hover:text-red-500">
                                <X className="w-4 h-4" />
                              </button>
                            )}
                          </div>
                    ...
                    <div className="bg-gray-50 rounded-xl p-3">
                      ...
                      {DOC_SLOTS.map(slot => (
                        <div key={slot.key} className="flex items-center gap-2 text-xs py-0.5">
                          <span className={docs[slot.key] ? 'text-green-500' : 'text-red-400'}>{docs[slot.key] ? '✓' : '✗'}</span>
