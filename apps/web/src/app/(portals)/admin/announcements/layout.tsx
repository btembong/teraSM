...
export default async function AnnouncementsLayout({ children }: { children: React.ReactNode }) {
  ...
  const total = tenantId
    ? await prisma.announcement.count({ where: { tenantId } })
    : 0

  const tabs = [
    { label: 'Announcements', href: '/admin/announcements', badge: total },
  ]
    ...
    <div>
      <div className="mb-8 -mt-1">
        ...
        <div className="flex items-center gap-3 mb-5">
          ...
          <div>
            <h1 className="text-xl font-bold text-gray-900 leading-none">Announcements</h1>
            <p className="text-sm text-gray-400 mt-0.5">Broadcast messages to students, staff, and parents</p>
          </div>
        </div>
