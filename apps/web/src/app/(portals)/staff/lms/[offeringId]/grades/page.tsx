...
import { GraduationCap, TrendingUp, CheckCircle2, XCircle, Minus, Download } from 'lucide-react'
import CsvImportButton from './_components/CsvImportButton'
...
export default async function StaffGradesPage({
  ...
  const studentRows = students.map(student => {
    ...
    const pct = totalMaxScore > 0 && graded > 0
      ? Math.round((totalScore / (graded * (totalMaxScore / assignments.length))) * 100)
      : null
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-gray-200 overflow-x-auto">
        <div className="px-6 py-4 border-b border-gray-100 flex items-center gap-2">
          ...
          <div className="ml-auto flex items-center gap-2">
            <CsvImportButton offeringId={offeringId} />
            <a
              href={`/api/staff/grades-export?courseOfferingId=${offeringId}`}
              download
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-100 bg-gray-50/50">
                ...
                <th className="px-6 py-3 font-medium text-gray-500 text-xs w-44">Progress</th>
                {assignments.slice(0, 6).map(a => (
                  <th key={a.id} className="text-center px-2 py-3 font-medium text-gray-400 text-xs" title={a.title}>
      ...
      {/* Academic grade entry is handled in /staff/courses/[offeringId]/grade-entry */}
    </div>
  )
}
