  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-500">Your conversations with students and staff</p>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-200">
        {conversations.length === 0 ? (
          <div className="text-center py-16">
            ...
            <p className="text-gray-400 text-sm mt-1">Students and colleagues can message you directly</p>
          </div>
        ) : (
          <div className="divide-y divide-gray-100">
            {conversations.map((conv) => {
              ...
              return (
                <Link key={conv.id} href={`/staff/messages/${conv.id}`} className="flex items-center gap-4 px-6 py-4 hover:bg-gray-50 transition-colors">
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 ${conv.type === 'GROUP' ? 'bg-blue-100' : 'bg-blue-100'}`}>
                    {conv.type === 'GROUP'
