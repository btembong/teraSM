import { auth } from '@/lib/auth'
import { prisma } from '@/lib/prisma'
import { NextRequest, NextResponse } from 'next/server'

type Params = { params: Promise<{ id: string }> }

export async function PATCH(req: NextRequest, { params }: Params) {
  const session = await auth()
  if (!session) return NextResponse.json({ message: 'Unauthorized' }, { status: 401 })
  const tenantId = (session.user as any).tenantId
  const { id } = await params

  const room = await (prisma as any).room.findFirst({ where: { id, tenantId } })
  if (!room) return NextResponse.json({ message: 'Not found' }, { status: 404 })

  const body = await req.json()

  // Check name uniqueness if changing
  if (body.name && body.name.trim() !== room.name) {
    const conflict = await (prisma as any).room.findFirst({
      where: { tenantId, name: body.name.trim(), NOT: { id } },
    })
    if (conflict) return NextResponse.json({ message: 'A room with this name already exists' }, { status: 409 })
  }

  const updated = await (prisma as any).room.update({
    where: { id },
    data: {
      ...(body.name         !== undefined ? { name: body.name.trim() }                   : {}),
      ...(body.code         !== undefined ? { code: body.code?.trim().toUpperCase() || null } : {}),
      ...(body.building     !== undefined ? { building: body.building?.trim() || null }  : {}),
      ...(body.floor        !== undefined ? { floor: body.floor?.trim() || null }        : {}),
      ...(body.capacity     !== undefined ? { capacity: body.capacity }                  : {}),
      ...(body.roomType     !== undefined ? { roomType: body.roomType }                  : {}),
      ...(body.hasProjector !== undefined ? { hasProjector: body.hasProjector }          : {}),
      ...(body.hasAC        !== undefined ? { hasAC: body.hasAC }                        : {}),
      ...(body.isActive     !== undefined ? { isActive: body.isActive }                  : {}),
    },
    include: { _count: { select: { offerings: true } } },
  })

  return NextResponse.json(updated)
}

export async function DELETE(_req: NextRequest, { params }: Params) {
  const session = await auth()
  if (!session) return NextResponse.json({ message: 'Unauthorized' }, { status: 401 })
  const tenantId = (session.user as any).tenantId
  const { id } = await params

  const room = await (prisma as any).room.findFirst({
    where: { id, tenantId },
    include: { _count: { select: { offerings: true } } },
  })
  if (!room) return NextResponse.json({ message: 'Not found' }, { status: 404 })
  if (room._count.offerings > 0) {
    return NextResponse.json(
      { message: `Cannot delete: this room is assigned to ${room._count.offerings} course offering(s). Reassign them first.` },
      { status: 400 },
    )
  }

  await (prisma as any).room.delete({ where: { id } })
  return NextResponse.json({ ok: true })
}
